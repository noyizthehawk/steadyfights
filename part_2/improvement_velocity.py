...
def get_top_careers_by_metric(top_n: int = 20, min_fights: int = 5):
    ...
    for fighter_name, group in fighters_df.groupby('name'):
        ...
        score = compute_career_score(
            fighter_fights, max_adj_perf,
            win_col='win_flag_indicator', perf_col='adjusted_performance',
        )
...
def get_fighter_stats(fighter):
    ...
    if len(fighter_fights) > 0:
        ...
        # Use the same  function as the GOAT list
        max_adj_perf = fighters_df['adjusted_performance'].max()
        career_quality_title_adjusted = compute_career_score(
            fighter_fights, max_adj_perf,
            win_col='win_flag_indicator', perf_col='adjusted_performance',
        )
...
if __name__ == "__main__":
    import sys
    # The opponent-strength CSV is generated by the module-level code above, which
    # runs on every execution. main() is a separate INTERACTIVE menu (GOAT list /
    # fighter stats), so only launch it when attached to a terminal. The batch
    # pipeline (refresh_data.py) runs this head-less; without this guard, main()'s
    # input() raises EOFError and fails the refresh after the data was already built.
    if sys.stdin.isatty():
        main()
    else:
        print("improvement_velocity: data regenerated (non-interactive run — menu skipped).")